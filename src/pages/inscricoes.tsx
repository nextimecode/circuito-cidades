...
const Inscricoes = () => {
  return (
    <Layout>
      <Section px={[4, 10]}>
        <Title label="Inscreva-se" alignSelf={['center', 'flex-start']} />
        <VStack spacing={2} alignItems="flex-start" py={[2, 4]}>
          <Text>
            1. Preencha o formulário de inscrição [link abaixo] ou diretamente no
            Departamento/Secretaria de Esportes de sua cidade;
          </Text>
          <Text>
        ...
        {listCities.map((city, index) => (
          <Box as="div" key={index} py={[2]}>
            <Text fontSize="lg" fontWeight={700} color="primary" py={1}>
              {city.title}
            </Text>
            <Text fontSize="sm">
              {city.info?.place}
